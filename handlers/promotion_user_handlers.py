...
async def offers_catalog(callback: CallbackQuery):
    ...
    with get_service(PromotionService) as promotion_service:
        ...
        if not promos:
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[[InlineKeyboardButton(text="🔙 Volver", callback_data="offers")]]
            )
            text = (
                "🎩 <b>Lucien:</b>\n\n"
...
async def view_offer_detail(callback: CallbackQuery, callback_data: ViewOfferCallback):
    ...
    with get_service(PromotionService) as promotion_service:
        ...
        if has_interest:
            text += (
                "<i>Ya ha expresado su interes en esta experiencia. "
                "Diana ha sido notificada y se pondra en contacto con usted...</i>\n"
            )
        elif is_blocked:
            text += (
                "<i>Su cuenta tiene ciertas... limitaciones que impiden expresar interes. "
...
async def express_interest(callback: CallbackQuery, callback_data: OfferInterestCallback, bot: Bot):
    ...
    with get_service(PromotionService) as promotion_service:
        ...
        if promotion_service.has_user_expressed_interest(user.id, promo_id):
            await callback.answer("Ya ha expresado interes en esta experiencia.", show_alert=True)
            return
            ...
            [
                InlineKeyboardButton(
                    ...
                    callback_data=BlockInterestCallback(
                        user_id=interest.user_id, confirmed=False
                    ).pack(),
                )
            ],
        ]
...
async def my_offers_history(callback: CallbackQuery):
    ...
    with get_service(PromotionService) as promotion_service:
        ...
        if not interests:
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="✨ Ver el Gabinete", callback_data="offers_catalog"
                        )
                    ],
        ...
        for interest in interests:
            ...
            status_emoji = {"pending": "⏳", "attended": "✅", "blocked": "🚫"}.get(

...
@router.callback_query(F.data == "streak_promo_create",
                       lambda cb: is_admin(cb.from_user.id))
...
@router.message(StreakPromotionStates.waiting_name,
...
async def streak_promo_get_name(message, state: FSMContext):
    ...
    if not name or len(name) > 100:
        await message.answer(
            "⚠️ El nombre debe tener entre 1 y 100 caracteres. Intente de nuevo:"
        )
        return
...
@router.message(StreakPromotionStates.waiting_description,
                lambda msg: is_admin(msg.from_user.id))
...
@router.message(StreakPromotionStates.waiting_level,
                lambda msg: is_admin(msg.from_user.id))
async def streak_promo_get_level(message, state: FSMContext):
    ...
    buttons = [
        [InlineKeyboardButton(
            text="✅ Si, agregar otro nivel",
            callback_data="streak_promo_add_level"
        )],
        [InlineKeyboardButton(
            text="\U0001f6ab No, continuar",
            callback_data="streak_promo_no_more_levels"
        )],
    ]
    await message.answer(
...
@router.callback_query(F.data == "streak_promo_add_level",
                       lambda cb: is_admin(cb.from_user.id))
...
@router.callback_query(F.data == "streak_promo_no_more_levels",
                       lambda cb: is_admin(cb.from_user.id))
async def streak_promo_choose_duration_mode(callback: CallbackQuery, state: FSMContext):
    ...
    buttons = [
        [InlineKeyboardButton(
            text="\U0001f4c5 Fechas concretas",
            callback_data="streak_promo_dur_dates"
        )],
        [InlineKeyboardButton(
...
@router.callback_query(F.data == "streak_promo_dur_dates",
                       lambda cb: is_admin(cb.from_user.id))
...
@router.message(StreakPromotionStates.waiting_start_date,
                lambda msg: is_admin(msg.from_user.id))
async def streak_promo_get_end_date(message, state: FSMContext):
    ...
    if text.lower() == "ahora":
        ...
        try:
            ...
            await message.answer(
                "⚠️ Formato invalido. Use <code>DD/MM/AAAA HH:MM</code> "
                "o <code>ahora</code>.",
                parse_mode="HTML"
            )
            return
...
@router.message(StreakPromotionStates.waiting_end_date,
                lambda msg: is_admin(msg.from_user.id))
...
@router.callback_query(F.data == "streak_promo_dur_relative",
                       lambda cb: is_admin(cb.from_user.id))
...
@router.message(StreakPromotionStates.waiting_duration_hours,
                lambda msg: is_admin(msg.from_user.id))
async def streak_promo_duration_hours_got(message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "⚠️ Ingrese un numero valido de horas positivo.",
            parse_mode="HTML"
        )
        return
...
async def streak_promo_toggle_game_type(callback: CallbackQuery, callback_data: TriviaStreakGoalTypeCallback, state: FSMContext):
    ...
    if flag == "done":
        ...
        confirm_buttons = [
            [InlineKeyboardButton(text="✅ Confirmar y crear", callback_data="streak_promo_confirm")],
            [InlineKeyboardButton(text="\U0001f504 Reiniciar", callback_data="streak_promo_create")],
            [InlineKeyboardButton(text="❌ Cancelar", callback_data="admin_streak_promotions")],
        ]
        await callback.message.edit_text(
            summary_text,
...
def streak_promo_build_summary(data: dict, game_types: dict) -> str:
    ...
    if mode == "dates":
        ...
        lines.append(f"<b>Duracion:</b> Fechas concretas")
        if start:
...
@router.callback_query(F.data == "streak_promo_confirm",
                       lambda cb: is_admin(cb.from_user.id))
...
def _build_promotion_detail_text(promo) -> str:
    ...
    status_icons = {"active": "\U0001f7e2", "paused": "\U0001f534",
                    "pending": "\U000023f3", "expired": "⚪"}
    ...
    game_types = [t for t, attr in [("General", promo.include_general),
                                     ("Simple", promo.include_simple),
                                     ("VIP", promo.include_vip)] if attr]
    lines.append(f"<b>Tipos:</b> {' + '.join(game_types)}")
    lines.append(f"\n<b>Niveles:</b>")
    for level in promo.levels:
        delivered = sum(1 for c in level.codes if getattr(c.status, 'value', str(c.status)) == "delivered")
        lines.append(

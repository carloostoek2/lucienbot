...
| `refactor_testing.md` + `fases_refactor_testing.md` | Actualizado | Estado top, tabla Archivos, nueva sección "Trabajo realizado en esta sesión (punto 7)", s.8 Cómo Retomar actualizada con handoff para siguiente (handler e2e callbacks, property tests costo, full chain trivia→streak→risk→cleanup, coverage). |
...
4. Próximos pasos recomendados (en orden de valor, actualizar al retomar):
   ...
   Siguientes: handler e2e completo para callbacks streak (accept/decline/retire/continue con mocks CallbackQuery + make_ factories de conftest), property tests para calculate_protection_cost + invariantes (nunca neg post protect), full integration chain trivia→claim code→offer retire/continue→set_risk→fail/timeout→cleanup, medición cobertura real post-slice para streak/game_service, error paths adicionales (concurrent, DB fail en debit protect, tz edges en expires aware/naive), full dice en risk si aplica. Future debt: modernize tz handling (utcnow/naive) across slices while preserving reference pattern replication (see item6/7 notes + test EOFs).

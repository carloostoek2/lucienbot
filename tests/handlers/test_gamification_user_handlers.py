...
class TestClaimDailyGift:
    ...
    async def test_calls_service_with_user_id(self, mock_get_service, make_callback):
        ...
        mock_instance.claim_gift_with_missions.assert_awaited_once_with(123456789, bot=cb.bot)
...
class TestHandleReaction:
    ...
    async def test_registers_reaction(self, mock_get_service, make_callback):
        """Llama a process_channel_reaction con parámetros correctos."""
        ...
        mock_instance.process_channel_reaction.assert_called_once()
        _, kwargs = mock_instance.process_channel_reaction.call_args
        ...
        assert kwargs["username"] == cb.from_user.username
        assert kwargs["bot"] is cb.bot
...
class TestCalculateEmojiCountsFromReactions:
    """Tests para el helper puro en keyboards.broadcast_channel_markup."""

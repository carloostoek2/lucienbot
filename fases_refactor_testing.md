...
|------|--------------------------|--------|--------|-------------------------------------|------------------|
| Pre-GSD | Gestión de Canales (Fundacional) | Reporte generado (recs open) | Jun 2026 | Primera revisión cronológica per docs/fase_testing_review_process.md (6 Pasos full). Subagentes: explore (019e862a-2c24-7f63-81fb-5988d093e34e) + impact-analyzer (019e862e-344c-7972-8f7c-a9fef72064e5) lanzados pre. Reverts git: prod docstrings/CLAUDE + added pilot in integ (to enforce ZERO prod + 'pilots documented only/not executed'). ID complete: conftest sample_pending + all unit creates/paths + DT aware (Issue 4). Files touched post: fases_refactor_testing.md, refactor_testing.md (minimal handoff), tests/unit/test_channel_service.py (ID+loose+DT), tests/conftest.py (fixture). GSD to existing log before every search. Source refs corrected to services/CLAUDE.md + handlers/CLAUDE.md + models/CLAUDE.md + services/channels/CLAUDE.md (domain). Hallazgos: ID duality (now fixed in tests), scheduler bypass, approve_all gap (Alta sacositas), handler rules, dt, doc drift, loose asserts. Report section tightened. All 18 review issues addressed (fixed/wontfix + Responses). Final ruff/pytest clean. Ver full report section below + /tmp/grok-impl-summary-ae9b25c5.md (with Fix round updates). | Alta pilots (approve gap, scheduler errors) follow-up per recs; Fase 07.1. |
...
- **Alta prioridad (mitiga sacositas stuck/ghost approvals, ID bugs):** 
  1. Alta, esfuerzo=medio, riesgo=ghost approvals + stuck joins: Nuevo pilot contrato en test_free_entry_flow.py (o extend): Test approve_all limitation (assert NO bot.approve llamado, DB updated pero simular re-join falla o estado TG).
  2. Alta, esfuerzo=alto, riesgo=phantom state + race approvals: Fortalecer/añadir variants scheduler error (inactive, send fail, rollback continue) + inactive channel full matrix usando patrón file SQLite+patch.
  3. Alta, esfuerzo=medio, riesgo=ID silent fails + tz flakes: Completar fix fixtures + estandarizar DT en todos tests canales (GSD + ruff post).

...
| [@.planning/HARDENING_ROADMAP.md] | Living cognitive "hoja de ruta" del hardener: Quick path (status + max 4 next) → Analysis status → Decisions core → pipeline → Done **index** (1 row/pool) → Gaps/Metrics. Narrative larga en SUMMARYs o [HARDENING_ROADMAP_HISTORY.md](.planning/HARDENING_ROADMAP_HISTORY.md). Documentador: refresh compacto (no paste de SUMMARY). Pool phrase verbatim once. |
...
- **Explicit launch of the documentador agent at the end of each pool** (after last item's test-guardian + tests green + self-check): updates `.planning/HARDENING_ROADMAP.md` in **compact form** (Quick path + 1 index row + Latest pool block ≤15 lines + Gaps refresh + phrase once; long narrative stays in SUMMARY/agent-memory), extracts learnings/patterns to documentador report, follows GSD pre-log to `.planning/quick/gsd-documentador-*.log` + wc -l.
